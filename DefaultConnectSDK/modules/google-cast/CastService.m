...
- (GCKDeviceManager *)createDeviceManagerWithDevice:(GCKDevice *)device
    ...
    // CRITICAL:
    // Google Cast SDK v2.x (2.7.1 in this project) auto-disconnects when the app is backgrounded
    // by default. That makes MPRemoteCommandCenter (Lock Screen / Control Center) commands fail
    // while the TV keeps playing, often surfacing as "There is no media currently available".
    //
    // We keep the connection lifecycle under our control by ignoring app state notifications.
    // The app already uses background audio to stay alive while casting.
