...
- (BOOL) connected
{
    // Không kiểm tra client-key, chỉ cần socket đã kết nối
...
-(void) showAlert
{
    ...
    if(self.pairingType == DeviceServicePairingTypePinCode || self.pairingType == DeviceServicePairingTypeMixed){
        ...
        _pairingAlert = alertController;
    }
    dispatch_on_main(^{ [self showAlertControllerOnTop: _pairingAlert]; });
}
-(void) showAlertWithTitle:(NSString *)title andMessage:(NSString *)message
{
    ...
    dispatch_on_main(^{ [self showAlertControllerOnTop:_pinAlertView]; });
}
-(void)dismissPinAlertView{
    UIViewController *topViewController = [UIApplication sharedApplication].keyWindow.rootViewController;
    while (topViewController.presentedViewController) {
        topViewController = topViewController.presentedViewController;
        if (([topViewController isKindOfClass:[UIAlertController class]]) && _pinAlertView) {
            NSLog(@"UIAlertController is currently visible.");
            break;
        }
    }
...
- (void)showAlertControllerOnTop:(UIAlertController *)alertController {
...
- (void)dismissAllAlertControllers {
    UIViewController *topViewController = [UIApplication sharedApplication].keyWindow.rootViewController;
    while (topViewController.presentedViewController) {
        topViewController = topViewController.presentedViewController;
        if ([topViewController isKindOfClass:[UIAlertController class]]) {
...
{
    if (_pairingAlert && [self isAnyAlertControllerVisible])
        dispatch_on_main(^{ [self dismissAllAlertControllers]; });
...
{
    NSLog(@"[ConnectSDK] WebOSTVService socketDidConnect called.");
    [_pairingTimer invalidate];
    if (_pairingAlert && [self isAnyAlertControllerVisible])
        dispatch_on_main(^{ [self dismissAllAlertControllers]; });
    
    // Luôn coi như thành công ngay cả khi không có client-key
    if ([self.delegate respondsToSelector:@selector(deviceServicePairingSuccess:)]) {
        NSLog(@"[ConnectSDK] Notifying pairing success");
...
- (void)closeApp:(LaunchSession *)launchSession success:(SuccessBlock)success failure:(FailureBlock)failure
{
    ...
    NSMutableDictionary *payload = [NSMutableDictionary new];
    if (launchSession.appId) [payload setValue:launchSession.appId forKey:@"id"];
...
- (void)displayImageWithMediaInfo:(MediaInfo *)mediaInfo success:(MediaPlayerSuccessBlock)success failure:(FailureBlock)failure {
    ...
    // Ưu tiên dùng DLNA nếu có hỗ trợ displayImageWithMediaInfo
    if (self.dlnaService && [self.dlnaService respondsToSelector:@selector(mediaPlayer)]) {
        id<MediaPlayer> mediaPlayer = [self.dlnaService performSelector:@selector(mediaPlayer)];
        if (mediaPlayer && [mediaPlayer respondsToSelector:@selector(displayImageWithMediaInfo:success:failure:)]) {
            [mediaPlayer displayImageWithMediaInfo:mediaInfo success:success failure:failure];
            return;
        }
    }

    // Nếu service hiện tại hỗ trợ displayMediaWithParams (trường hợp WebOS truyền thống)
    if ([self respondsToSelector:@selector(displayMediaWithParams:success:failure:)]) {
        NSDictionary *params = @{
    ...
    // Fallback: dùng Web App "MediaPlayer"
    ...
    WebAppLaunchSuccessBlock connectSuccess = ^(WebAppSession *webAppSession) {
        ...
        if (session.mediaPlayer && [session.mediaPlayer respondsToSelector:@selector(displayImageWithMediaInfo:success:failure:)]) {
            [session.mediaPlayer displayImageWithMediaInfo:mediaInfo success:success failure:failure];
        } else {
            if (failure) {
                NSError *error = [NSError errorWithDomain:@"com.yourapp.cast"
                                                     code:-1
                                                 userInfo:@{NSLocalizedDescriptionKey : @"WebAppSession does not support displayImageWithMediaInfo"}];
                failure(error);
            }
        }
    };
...
- (void)playMediaWithMediaInfo:(MediaInfo *)mediaInfo shouldLoop:(BOOL)shouldLoop success:(MediaPlayerSuccessBlock)success failure:(FailureBlock)failure {
    ...
    // Ưu tiên dùng DLNA nếu có hỗ trợ mediaPlayer
    if (self.dlnaService && [self.dlnaService respondsToSelector:@selector(mediaPlayer)]) {
        id<MediaPlayer> mediaPlayer = [self.dlnaService performSelector:@selector(mediaPlayer)];
    ...
    // Nếu không có DLNA hoặc displayMedia, fallback dùng WebAppSession (MediaPlayer Web App)
    NSString *webAppId = @"MediaPlayer";
    WebAppLaunchSuccessBlock connectSuccess = ^(WebAppSession *webAppSession) {
        WebOSWebAppSession *session = (WebOSWebAppSession *)webAppSession;

        if (session.mediaPlayer && [session.mediaPlayer respondsToSelector:@selector(playMediaWithMediaInfo:shouldLoop:success:failure:)]) {
            [session.mediaPlayer playMediaWithMediaInfo:mediaInfo shouldLoop:shouldLoop success:success failure:failure];
        } else {
            if (failure) {
                NSError *error = [NSError errorWithDomain:@"com.yourapp.cast"
                                                     code:-1
                                                 userInfo:@{NSLocalizedDescriptionKey : @"WebAppSession does not support mediaPlayer"}];
                failure(error);
            }
        }
    };
    ...
    [self joinWebAppWithId:webAppId success:connectSuccess failure:^(NSError *error) {
        // Nếu join thất bại, thử launch WebApp
        [self launchWebApp:webAppId success:connectSuccess failure:failure];
    }];
}
